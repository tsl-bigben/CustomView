...
final class SmartTabStrip extends LinearLayout {
    ...
    private static final boolean DEFAULT_INDICATOR_TRIANGLE = false;//是否是三角形
    ...
    private final boolean indicatorTriangle;
    ...
    SmartTabStrip(Context context, AttributeSet attrs) {
        ...
        boolean indicatorTriangle = DEFAULT_INDICATOR_TRIANGLE;
        ...
        indicatorTriangle = a.getBoolean(R.styleable.stl_SmartTabLayout_stl_triangle, indicatorTriangle);
        a.recycle();
        ...
        this.indicatorTriangle = indicatorTriangle;
    ...
    public void setCustomTabColorizer(SmartTabLayout.TabColorizer customTabColorizer) {
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
        ...
        if (indicatorWidth == AUTO_WIDTH) {
            if(indicatorTriangle){
                float t_x = (right - left) * 1.00f / 2 + left;
                mPath.reset();
                mPath.moveTo(left, bottom);
                mPath.lineTo(right, bottom);
                mPath.lineTo(t_x, top);
                mPath.close();
            } else {
            ...
            if(indicatorTriangle) {
                float b_left = left + padding;
                float b_right = right - padding;
                float t_x = (b_right - b_left) * 1.0F / 2 + b_left;
                mPath.reset();
                mPath.moveTo(b_left, bottom);
                mPath.lineTo(b_right, bottom);
                mPath.lineTo(t_x, top);
                mPath.close();
            } else {
        ...
        if(indicatorTriangle){
            if(mPath != null) {
                canvas.drawPath(mPath, indicatorPaint);
            }
        } else {
